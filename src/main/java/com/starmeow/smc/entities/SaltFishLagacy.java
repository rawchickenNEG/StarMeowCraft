...
public class SaltFishLagacy extends TamableAnimal implements Bucketable {
    protected static final EntityDataAccessor<Optional<UUID>> OWNER_UUID = SynchedEntityData.defineId(SaltFishLagacy.class, EntityDataSerializers.OPTIONAL_UUID);
    private static final EntityDataAccessor<Boolean> FROM_BUCKET = SynchedEntityData.defineId(SaltFishLagacy.class, EntityDataSerializers.BOOLEAN);
    public SaltFishLagacy(EntityType<SaltFishLagacy> p_28276_, Level p_28277_) {
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return SaltFishLagacy.createMobAttributes()
    ...
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob p_146744_) {
        SaltFishLagacy saltFish = EntityTypeRegistry.SALT_FISH.get().create(level);
        if (saltFish != null) {
    ...
    public boolean wantsToAttack(LivingEntity target, LivingEntity owner) {
        if (!(target instanceof Creeper) && !(target instanceof Ghast)) {
            if (target instanceof Player && owner instanceof Player && !((Player)owner).canHarmPlayer((Player)target)) {
                ...
                return false;
            } else if (target instanceof SaltFishLagacy && ((SaltFishLagacy)target).isTame()) {
                return false;
            } else {
    ...
    public static class SaltFishSwimmingGoal extends RandomSwimmingGoal {
        public final SaltFishLagacy fish;
        public SaltFishSwimmingGoal(SaltFishLagacy p_27505_) {
    ...
    public static class SaltFishFollowOwnerGoal extends Goal {
        ...
        public SaltFishFollowOwnerGoal(SaltFishLagacy fish) {
            this.fish = fish;
    ...
    static class SaltFishMoveControl extends MoveControl {
        private final SaltFishLagacy fish;

        SaltFishMoveControl(SaltFishLagacy p_27501_) {
            super(p_27501_);
            this.fish = p_27501_;
        }
